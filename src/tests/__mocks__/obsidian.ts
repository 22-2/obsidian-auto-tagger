	...
	addButton(callback: (button: any) => void): Setting {
		const mockButton = {
			...
			setTooltip: (text: string) => {
				mockButton.buttonEl.title = text;
				return mockButton;
			},
			setCta: () => mockButton,
...
export class ButtonComponent {
	...
	setTooltip(tip: string) {
		this.buttonEl.setAttribute("data-tooltip", tip);
		return this;
	}
	setClass(cls: string) {
...
export class DropdownComponent {
	...
	addOptions(options: Record<string, string>) {
		Object.entries(options).forEach(([v, l]) => this.addOption(v, l));
		return this;
	}
...
export const requestUrl = jest.fn().mockImplementation(async (options: any) => {
	// A basic mock implementation for requestUrl
	// It can be overridden in specific tests if needed
	return Promise.resolve({
		status: 200,
		headers: {},
...
export const Menu = jest.fn().mockImplementation(() => ({
	items: [],
	addItem: jest.fn().mockImplementation(function (
		this: any,
...
export const MockObsidian = {
	...
	createTestFile: (path: string, content: string) =>
		mockFileSystem.create(path, content),
