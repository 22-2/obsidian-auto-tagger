...
check_prerequisites() {
    ...
    if ! command -v npm &> /dev/null; then
        log_error "Error: 'npm' is not installed. Please make sure it is available in the environment."
    fi
...
unpack_obsidian_assets() {
    ...
    npm exec asar extract "${APP_ASAR_PATH}" "${OBSIDIAN_UNPACKED_PATH}"
...
build_plugin() {
    ...
    if ! (cd "$PLUGIN_SOURCE_FULL_PATH" && npm build); then
